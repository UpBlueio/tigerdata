...
# Install TimescaleDB extension package and toolkit for the target PostgreSQL version
RUN apt-get update && apt-get install -y --no-install-recommends \
...
# Copy timescaledb-toolkit extension artifacts
# Includes compiled .so library and SQL migration files
COPY --from=timescaledb-builder \
   /usr/lib/postgresql/${PG_MAJOR}/lib/timescaledb_toolkit*.so \
   /usr/lib/postgresql/${PG_MAJOR}/lib/
COPY --from=timescaledb-builder \
   /usr/share/postgresql/${PG_MAJOR}/extension/timescaledb_toolkit*.sql \
   /usr/share/postgresql/${PG_MAJOR}/extension/
COPY --from=timescaledb-builder \
   /usr/share/postgresql/${PG_MAJOR}/extension/timescaledb_toolkit.control \
   /usr/share/postgresql/${PG_MAJOR}/extension/
